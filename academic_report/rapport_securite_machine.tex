...
\author{Projet de vision par ordinateur pour la sécurité machine}
\date{Mai 2026}
...
\subsection{Architectures retenues}
Les deux familles utilisent les mêmes types de variables d'entrée: coordonnées de pose, visibilité, distances à la zone dangereuse, indicateurs d'intérieur de zone, vitesse et accélération. Elles ne répondent pas exactement au même usage opérationnel. Le TCN est retenu pour l'alerte opérateur avec un score unique de risque visible maintenant. Le GRU exact-entry est gardé pour la question plus étroite de l'arrêt court horizon, quand on veut savoir si l'entrée physique est devenue imminente à quelques dixièmes de seconde près.

\paragraph{TCN60 comme modèle d'alerte retenu.}
Le TCN60 reçoit une fenêtre causale de $60 \times 54$ features, soit environ deux secondes d'historique. Il applique une projection \texttt{Conv1d} $54 \rightarrow 96$, puis quatre blocs TCN résiduels avec dilatations $1,2,4,8$. Chaque bloc contient deux convolutions 1D, \texttt{BatchNorm}, \texttt{ReLU} et un dropout de $0,18$. La tête concatène la moyenne temporelle et le dernier pas temporel, puis applique un MLP $192 \rightarrow 96 \rightarrow 1$. La sortie unique est \texttt{risk\_present\_now}. Dans nos résultats, la variante \texttt{tcn60\_aug\_bce} donne le meilleur compromis entre couverture des entrées et faible bruit.
...
Le score d'alerte n'est pas choisi à partir d'un seul entraînement isolé. Pour la sortie unique \texttt{risk\_present\_now}, nous avons comparé plusieurs variantes directes sur les mêmes splits répétés:
\begin{itemize}[leftmargin=*]
...
\end{itemize}
Séparément, pour la question de stop court horizon, nous avons gardé une expérience GRU exact-entry, évaluée comme signal d'imminence et non comme alerte opérateur.
...
\textbf{BCE} signifie \emph{Binary Cross Entropy}. C'est la perte standard pour une sortie binaire comme \texttt{risk\_present\_now}. Elle pénalise directement les erreurs entre la probabilité prédite et le label 0/1. Nous l'avons gardée parce qu'elle donne une baseline stable et facile à interpréter. Dans nos entraînements, elle est utilisée avec un poids positif pour compenser le fait que les fenêtres positives sont beaucoup moins nombreuses que les fenêtres négatives.
...
\]
Ces variantes sont utiles pour comparer des politiques d'alerte plus agressives. La conclusion retenue reste prudente: attention et blouse/PPE modifient la sensibilité de l'alerte opérateur, mais le danger physique reste porté par la séquence pose-zone.
...
\midrule
TCN60 retenu, seuil 0,55 & 0.850 & 0.642 & 0.859 & 1.574 & 0.394 s \\
TCN60 plus agressif, seuil 0,35 & 0.850 & 0.683 & 0.661 & 6.015 & 0.456 s \\
\bottomrule
\end{tabular}
}
\end{table}

Cette table présente deux points de fonctionnement du TCN d'alerte. Le seuil $0,55$ offre le meilleur équilibre pour une alerte opérateur continue: l'avance utile reste élevée et le bruit reste contenu. Le point plus agressif gagne légèrement à $0,3$ s, mais au prix d'une hausse marquée des fausses alertes.
